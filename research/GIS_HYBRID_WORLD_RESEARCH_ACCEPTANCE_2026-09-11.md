...
| **Sidewalk polygon** | City of Boston **Sidewalk Inventory** (OpenData MapServer/0) — **[UPDATED 2026-09-12]** | MassGIS | OSM | — | **ODC-PDDL-1.0**, pinned via the Analyze Boston record naming that exact layer | **68 polygons** in the Stage 1A box; geometry quality **unassessed**; **2014** PWD survey | **AUTHORITATIVE CANDIDATE FOUND — QUALITY / VINTAGE FITNESS UNVALIDATED.** Found in Stage 1A. Existing is not the same as fit: do not promote a 2014 layer to canonical production input without validating it. |
