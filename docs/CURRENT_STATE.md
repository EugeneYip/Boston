...
| Daylight hue — CANDIDATE | **NEEDS FRESH ATTRIBUTION, not investigated.** `st_southend` in daylight measures ~R 111.5 / G 105.3 / B 111.2, i.e. **R > B > G**. Recorded as a candidate only, **not** established as the same cause as dusk. **Correction (static audit, `b12497d`): an earlier version of this note said `ColorGrade` was NOT implicated, reasoning that B2 touched only the 19.7 keyframe and so cannot have caused it. That is a non-sequitur — B2's innocence does not exonerate `ColorGrade`, and the note risked steering a session away from the prime suspect.** Statically, the daylight keys carry the *same* B > G authoring pattern B2 diagnosed at dusk: `shadowTint` blue over green at 8.2 `[0.83,0.93,1.18]`, 11.6 `[0.93,0.97,1.10]`, 15.6 `[0.92,0.97,1.12]`. Treat `ColorGrade`'s daylight keys as the **first** hypothesis to test, not an excluded one — but still reproduce on current pixels and attribute by ablation before acting. |
