...
- **`prop:parkedCar` is a static shell, not a vehicle.** Nine variants, 424 tris at LOD0 /
  84 at LOD1, real dimensions, no physics body and no simulation. If traffic wants parked
  cars that can be driven away, take them over the way `traffic.takeOver` does.
