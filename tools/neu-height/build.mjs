...
fs.writeFileSync(path.join(OUT, 'HEIGHT_MEASUREMENTS.json'), JSON.stringify({
  ...
  _SUPERSEDED: {
    verdict: 'These heights are systematically HIGH. Do not use them as individual named-building heights.',
    useInstead: 'docs/neu/HERO_FOOTPRINTS.json (Boston Buildings with Roof Breaks, PDDL)',
    evidence: 'Boston 3D reads 233.6 m for the Prudential against a known 228 m, and +3.2 to +8.8 m across the Northeastern opening cluster. Its Height_Ft takes the top of the whole modelled mass, which is defensible on a merged object and wrong for a named building.',
    krentzmanCorrection: 'The quadrangle measures ~18.5 m, NOT the ~24.8 m the rows below imply.',
    stillUsefulFor: 'post-2010 buildings absent from the 2010 Roof Breaks snapshot (ISEC, EXP, East Village), and as a secondary cross-check — always as an upper bound.',
  },
